...
def is_logged_in():
    ...
    if not session_cookie:
        print("[WARN] Missing Firebase session cookie")
        return False
    try:
        decoded_claims = fb_auth.verify_session_cookie(session_cookie, check_revoked=False)
        session["admin_email"] = decoded_claims.get("email")
        return True
    except Exception as e:
        print("[WARN] Invalid or expired Firebase cookie:", e)
        return False
...
def session_login():
    ...
    try:
        ...
        # ✅ Use SameSite=None + Secure for modern browsers
        response.set_cookie(
            SESSION_COOKIE_NAME,
            session_cookie,
            ...
            httponly=True,
            samesite="None",
            secure=False  # ⚠️ set True if HTTPS
        )

        print("[INFO] ✅ Firebase session cookie created and set.")
        return response
